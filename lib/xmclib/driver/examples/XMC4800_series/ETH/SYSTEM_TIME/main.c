...
{
  uint32_t status = XMC_ETH_MAC_GetEventStatus(&ethif.mac);
  XMC_ETH_MAC_ClearEventStatus(&ethif.mac, XMC_ETH_MAC_EVENT_TIMESTAMP | XMC_ETH_MAC_EVENT_RECEIVE);

  if (status & XMC_ETH_MAC_EVENT_TIMESTAMP)
  {
    XMC_GPIO_SetOutputHigh(LED2);

    XMC_ETH_MAC_GetPTPStatus(&ethif.mac);
  }
...
{  
  ...
  XMC_ETH_MAC_SetPTPAlarm(&ethif.mac, &alarm);

  XMC_ETH_MAC_InitPTP(&ethif.mac, XMC_ETH_MAC_TIMESTAMP_CONFIG_FINE_UPDATE | XMC_ETH_MAC_TIMESTAMP_CONFIG_ENABLE_TS_INTERRUPT);
  XMC_ETH_MAC_EnableEvent(&ethif.mac, XMC_ETH_MAC_EVENT_TIMESTAMP);
