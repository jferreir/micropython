...
int8_t bx = 0;
static void buttons_task(void const *arg)
{
  XMC_UNUSED_ARG(arg);
  ...
  while(1)
  {
    ...
    osDelay(BUTTONS_TMR_INTERVAL);
  }
}
osThreadDef(buttons_task, osPriorityNormal, 1, 0);
...
void main_task(void const *args)
{
  ...
  osThreadCreate(osThread(buttons_task), NULL);
}
